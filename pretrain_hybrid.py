...
# Some Slurm/Pyxis launches reach distributed barriers before Megatron sets
# the rank-local CUDA device. Set it from Slurm early to avoid all ranks
# defaulting to cuda:0 during NCCL barrier setup.
_local_rank = os.environ.get("LOCAL_RANK", os.environ.get("SLURM_LOCALID"))
if _local_rank is not None and torch.cuda.is_available():
    torch.cuda.set_device(int(_local_rank) % torch.cuda.device_count())
