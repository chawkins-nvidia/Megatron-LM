...
def parse_and_validate_args(extra_args_provider=None, ignore_unknown_args=False, args_defaults={}):
    ...
    argparse_defaults = getattr(args, '_argparse_defaults', None)
    ...
    if args.yaml_cfg is not None:
        ...
        if argparse_defaults is not None:
            for key, value in vars(argparse_defaults).items():
                if key.startswith('_'):
                    continue
            ...
            for key in (
                ...
                if hasattr(args, key) and not isinstance(getattr(args, key), torch.dtype):
                    setattr(args, key, dtype_map[getattr(args, key)])
    else:
...
def parse_args(extra_args_provider=None, ignore_unknown_args=False):
    ...
    if args.yaml_cfg is not None:
        ...
        argparse_defaults = args
        args = load_yaml(args.yaml_cfg)
        args._argparse_defaults = argparse_defaults


    # Args from environment
    args.rank = int(os.getenv('RANK', '0'))
    args.world_size = int(os.getenv("WORLD_SIZE", '1'))
    env_local_rank = os.getenv("LOCAL_RANK", os.getenv("SLURM_LOCALID"))
...
def _add_network_size_args(parser):
    exclude = [
        ...
        "bias_dropout_fusion",
        "apply_rope_fusion",
        # legacy explicit CLI argument below
        "use_flash_attn",
    ]
