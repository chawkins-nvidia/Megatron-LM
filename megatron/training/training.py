...
def pretrain(
    ...
    if _STARTUP_TIMESTAMPS['program_start'] is not None:
        program_start_global = _STARTUP_TIMESTAMPS['program_start']
    ...
    if program_start is not None and main_entry is not None and pretrain_entry is not None:
        ...
        if os.environ.get('MEGATRON_SKIP_STARTUP_TIMER_COLLECTIVE', '').lower() in (
            '1',
            'true',
            'yes',
        ):
            startup_timer_msg = 'startup local times (ms):'
            for name, delta in startup_timers.items():
                startup_timer_msg += '\n    {}: {:.2f}'.format(
                    (name + ' ').ljust(48, '.'), delta * 1000.0
                )
            print_rank_0(startup_timer_msg)
        else:
