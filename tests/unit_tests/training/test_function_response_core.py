...
def test_zero_local_rows_are_neutral_and_preflight_rebinds_only_after_reset() -> None:
    ...
    widths = {family: 1 for family in RESPONSE_FAMILIES}
    probe.bind_preflight(
        selected_row_capacity=0,
        response_widths=widths,
        response_dtype=torch.float32,
        attention_heads=1,
        attention_key_length=1,
    )
    with pytest.raises(RuntimeError, match="preflight binding changed"):
        probe.bind_preflight(
            selected_row_capacity=1,
            response_widths=widths,
            response_dtype=torch.float32,
    ...
    for phase in ("pre", "post", "post_repeat", "midpoint"):
        probe.set_masks(torch.zeros((1, 1), dtype=torch.bool))
        with probe.capture_endpoint(phase):
            for module in modules:
                module(torch.ones((1, 1, 1)))

    probe.finalize()
    for descriptor in descriptors:
        for phase in ("pre", "post", "post_repeat", "midpoint"):
            (rows,) = probe.secant_endpoint_rows(phase, descriptor.key)
